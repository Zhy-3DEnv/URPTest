Shader "Framework/SFX/shd_additiveDistortionScroll_CustomDate_v2" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(v2f i) : COLOR {
                ...
                fixed3 emissive = mainTexVar.rgb * i.vertexColor.rgb * _ColorMultiplier * _TintColor.rgb * colorRampVar.rgb;
                emissive *= 2.0 * mainTexVar.a * pow(gradientVar.r, clamp(_GradientPower * i.uv_CustomData.x, 0, 10));
                
                float alpha = _Alpha;
                return saturate(fixed4(emissive * alpha, 1));
            }
            ENDHLSL
        }
    }
