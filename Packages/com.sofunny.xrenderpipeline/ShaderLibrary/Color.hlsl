#ifndef XRP_COLOR_INCLUDED
#define XRP_COLOR_INCLUDED

#include "Packages/com.sofunny.xrenderpipeline/ShaderLibrary/ACES.hlsl"
...
real3 UchimuraTonemap(real3 x) {
    // Uchimura 2017, "HDR theory and practice"
    // Math: https://www.desmos.com/calculator/gslcdxvipg
    // Source: https://www.slideshare.net/nikuque/hdr-theory-and-practicce-jp

    const real P = 1.0;  // max display brightness
    const real a = 1.0;  // contrast
    const real m = 0.22; // linear section start
    const real l = 0.4;  // linear section length
    const real c = 1.33; // black
    const real b = 0.0;  // pedestal

    const real l0 = ((P - m) * l) / a;
    const real S0 = m + l0;
    const real S1 = m + a * l0;
    const real C2 = (a * P) / (P - S1);
    const real CP = -C2 / P;

    real3 w0 = 1.0f - smoothstep(0.0f, m, x);
    real3 w2 = step(m + l0, x);
    real3 w1 = 1.0f - w0 - w2;

    real3 T = m * pow(x / m, c) + b;
    real3 S = P - (P - S1) * exp(CP * (x - S0));
    real3 L = m + a * (x - m);
