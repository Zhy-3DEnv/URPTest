...
namespace Framework.XRenderPipeline {
    ...
    sealed class ColorLookupEditor : VolumeComponentEditor {
        ...
        public override void OnInspectorGUI() {
            ...
            if (asset != null) {
                if (asset.useHDR && asset.colorGradingMode == ColorGradingMode.HighDynamicRange) {
                    EditorGUILayout.HelpBox("Color Grading Mode in the XRP Settings is set to HDR. As a result, this LUT will be applied after the internal color grading and tonemapping have been applied.", MessageType.Info);
                } else {
                    EditorGUILayout.HelpBox("Color Grading Mode in the XRP Settings is set to LDR. As a result, this LUT will be applied after tonemapping and before the internal color grading has been applied.", MessageType.Info);
                }
            }
        }
    }
